...
if (config.github.privateToken) {
  githubClient.authenticate({
    scopes: ['repo'],
    type: 'token',
    token: config.github.privateToken
  });
}

// extend with all() method
githubClient.issues.repoIssues.all = function (params, cb) {
  var issues = [];
  params.page = 1;
  params.per_page = 100;

  function fetchNextPage() {
    githubClient.issues.repoIssues(params, function (err, _issues) {
      if (err) {
        cb(err, issues);
        return;
      }
...
function syncServicesWithProjectIssues(project)
{
  ...
  return new Promise(function (resolve, reject) {
    ...
    Promise.series([
      utils.promisify(githubClient.issues.repoIssues.all, githubClient.issues, {
        user: parts[0],
        repo: parts[1],
        state: 'all'
      }),
